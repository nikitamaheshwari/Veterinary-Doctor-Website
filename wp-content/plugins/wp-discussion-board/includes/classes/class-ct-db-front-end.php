	...
	class CT_DB_Front_End {
			...
			if( 'POST' == $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['insert_post'] ) ) {
				...
				if( count( $errors ) != 0 ) {
					...
					if( $my_post_id !== false ) {
						...
						if( $status == 'publish' ) {
							$output .= '<p>' . wp_strip_all_tags( $_POST['topic_title'] ) . __( ' successfully added. View it at:', 'wp-discussion-board' ) . '<br>';
							$url = get_permalink( $my_post_id );
							$output .= '<a href="' . esc_url( $url ) .'">' . $url . '</a></p>';
						} else {
							$output .= '<p>' . $title . __( ' is awaiting moderation', 'wp-discussion-board' ) . '</p>';
						}
					}					
				}
			} else {
		...
		public function new_topic_form_content() {
			...
			if( isset( $_POST['topic_title'] ) ) {
				$title = wp_strip_all_tags( $_POST['topic_title'] );
			}
			$content = '';
			if( isset( $_POST['topic_content'] ) ) {
				$content = strip_shortcodes( wp_kses_post( $_POST['topic_content'] ) );
			}
			...
			$form['fields'] = array();
			$form['fields']['title'] = '<input class="required" type="text" id="topic_title" value="' . $title . '" tabindex="900" name="topic_title" placeholder="Topic Title" />';
			$form['fields']['content'] = '<textarea id="topic_content" name="topic_content" cols="80" rows="20" tabindex="901" placeholder="Topic Content">' . $content . '</textarea>';
